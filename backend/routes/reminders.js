...
async function generateSmartReminders(assignments) {
  ...
  const assignmentList = assignments
    // CORRECTED: Use 'deadline' from the database, map it to 'dueDate' for the prompt
  ...
  try {
    ...
    // Attempt to parse the JSON. Gemini might sometimes include markdown fences.
    ...
    // Return a default set of reminders or an empty array on error
    return [
      {
...
router.get("/smart", async (req, res) => {
  ...
  try {
    ...
    // CORRECTED: Changed 'a.due_date' to 'a.deadline' in the SELECT statement
    const assignmentsResult = await db.query(
      `SELECT a.id, a.title, a.deadline, a.status, s.name AS subject_name
       FROM assignments a
       JOIN subjects s ON a.subject_id = s.id
       WHERE a.user_id = ? AND a.status != 'submitted'
       ORDER BY a.deadline ASC`, // Also order by deadline
      [userId],
    )
    const assignments = assignmentsResult.rows || assignmentsResult // Adjust based on your db utility's return structure
